...
public class ControllerRight extends AbstractWindow {
    ...
    public void createList(){
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            HBox hBox = new HBox();
            Button enter = new Button("На подтверждение");
            ...
            enter.setOnAction(event -> {
                try {
                    connect.getStatement().executeUpdate("UPDATE CONTRACT SET CONFIRM = 'F' WHERE ID = " + id);
                } catch (SQLException e) {
                    e.printStackTrace();
                }
                createList();
            ...
            delete.setOnAction(event -> {
                ...
                ConfirmWindow.controllerRight.createList();
                createList();
            });
        }
    }
